...
export default function Navbar() {
  ...
  const linkVariants = {
    ...
    visible: (i) => ({
      opacity: 1,
      y: 0,
      transition: { delay: i * 0.07 + 0.15, ease: [0.22, 1, 0.36, 1], duration: 0.5 },
    }),
  };
  ...
  return (
    <>
      ...
      <nav
        ...
        className={`fixed z-[999] w-full px-4 sm:px-8 md:px-12 lg:px-20 py-5 md:py-8 flex justify-between items-center transition-all duration-300 ${
          scrolled || menuOpen
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-6 lg:gap-10">
          {navLinks.map((item, index) => (
            <a
              key={index}
              href="#"
              className={`text-sm lg:text-base font-light capitalize relative group transition-opacity duration-200 hover:opacity-70 ${
                index === navLinks.length - 1 ? "md:ml-8 lg:ml-16 xl:ml-32" : ""
              }`}
            >
              {item}
        ...
        <button
          className="md:hidden flex flex-col gap-1.5 p-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-white rounded"
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="fixed inset-0 z-[998] bg-zinc-900 flex flex-col items-center justify-center gap-8 md:hidden"
            role="dialog"
            aria-modal="true"
          ...
          >
            {navLinks.map((item, i) => (
              <motion.a
                ...
                className="text-4xl font-light capitalize text-white hover:text-[#CDEA68] transition-colors duration-200"
              >
                {item}
              </motion.a>
            ))}
